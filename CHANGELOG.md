...
### ⚠️ CRITICAL BLOCKER
- **DO NOT SHIP:** `mobile/main.js` contains `var UNLOCK_ALL = true;` hardcoded at the top of the file. This unlocks every politician in the roster AND waives the mobile phone install gate. This was added for local playtesting and must be reverted to `false` before deploying v2.6.0 or any later version to the live site. A `false` setting is the default and correct state.

### Added
- **ADDED**: `mobile/engine.js` `dominanceProgress(group, states, pop, player, thresholdBps)` — computes the seat-weighted fraction of a group's total seats held by a player (returns 0 to 1, and 0 — not NaN — for a group with no members, which is asserted in the self-check); mirrors the same `dominanceActive()` gate but measures progress rather than all-or-nothing completion. Called at render time to drive the new group-chip completion ring gauge. Includes self-check assertions in `node mobile/engine.js`.
- **CHANGED**: `mobile/main.js` `renderGroupCaptureBadges()` (pre-existing function, reworked) — group chips now display a real-time animated two-layer conic-gradient ring border, showing each player's seat-weighted progress toward completing the group. P1 fills clockwise from top, P2 fills anticlockwise; an unclaimed remainder is transparent (no black border). Module-level `prevCapturedP1` map tracks per-group capture state to edge-detect false→true transitions for sound firing (see below). Deliberately weighted by seats, not state count, after playtesting feedback: the ring now shows how much of a group's VALUE you hold, not how close to clearing the all-or-nothing gate — a 99%-held ring can be one 1-seat holdout from paying nothing, an accepted trade-off.
- **ADDED**: `sounds/group_won.wav` — new 32kHz mono 16-bit PCM synthesized sound clip (0.80s, -13.0 dBFS RMS) for group-capture events, replacing the repurposed `fanfare` which already signals "rally held." Fires at the moment a group's capture status transitions from false→true (edge-detect via `renderGroupCaptureBadges()`), once per capture, distinctly from the existing fanfare. Registered in `sounds` map before `unlockSounds()` runs, so the iOS per-element gesture-unlock gate covers it (see the unlock-paths finding in findings.md). **Note (2026-09-02):** Still unresolved as silent on a real phone while correctly audible in Chromium; see findings.md [F8] for next diagnostics.
- **ADDED**: `mobile/balance-sim.js` optional 2nd CLI argument forces both seats onto a named AI ladder rung for isolated profile testing (e.g., `node mobile/balance-sim.js 10 level-8`); omitting the rung keeps the previous behaviour, in which both seats fall back to `ai.js`'s `DEFAULT_RUNG` (`level-3`) — not random or adaptive (see CHANGELOG v2.5.0 for the ladder definition).

### Changed
- **CHANGED**: `data/states_data.json` — `Manufacturing` regional-dominance group re-grouped from 7 to 9 states based on MoSPI's top-10-by-registered-factories list. Removed West Bengal (not in top 10), added Punjab, Rajasthan, and Andhra Pradesh (all three in top 10, cleaned cancellations via policy magnitude adjustments below). `Education` group gained Assam (previously in neither group). Context: the two groups shared 5 of ~7 states (Jaccard 0.50) before this change — both mapped to the same "modern urban" pole thematically and were structurally near-synonyms. Manufacturing now has Education-unique states (Assam) and Manufacturing-unique states (Punjab, Rajasthan, Andhra Pradesh), reducing overlap from Jaccard 0.50 to 0.29 and making both groups harder to capture independently (see findings.md [F6]: expanding a group's member count increases difficulty because each member is an AND-gate veto point). Seat counts now symmetric: Education-only 5 states / 84 seats, Manufacturing-only 5 states / 99 seats.
- **CHANGED**: `data/policy-tags.json` — four `Manufacturing`-opposing policies stepped down one rung in magnitude to prevent same-state cancellations introduced by the new states: `Rural Development` -8 → -4 (opposes Agricultural Region), `Land Reforms` +8 → +4 (opposes Manufacturing), `Agricultural Reforms` -8 → -4 (opposes Agricultural Region), `Economic Liberalization` +12 → +8 (opposes Agricultural Region). Rationale: these four policies carry both Manufacturing and AgriculturalRegion tags at matched magnitudes (a structural polarity the tag system enforces across 9 policies); adding agricultural states to Manufacturing creates a cancellation for each pair. Lowering the Manufacturing-opposed magnitude in these four is asymmetric but cleaner than raising AgriculturalRegion elsewhere, which would propagate collisions into Haryana, Maharashtra, Telangana and Madhya Pradesh (see findings.md [F3] for the full brute-force audit). National policy-kit averages shifted ≤1.3 points across all politicians — negligible.
- **CHANGED**: `mobile/main.js` — group chips sorted ascending by total seats (smallest first, e.g. Coastal India's 156 seats → Northeast's 228 seats) instead of descending. Implemented as a sort on a shallow copy of `game.groups` (critical: `mobile/ai.js` indexes `game.groups` positionally for its target picks, so an in-place sort would alter AI behaviour and invalidate replay logs). User preference from playtesting.
- **CHANGED**: `mobile/index.html` `.gchip .hex` rewritten as a two-layer conic-gradient ring border over a transparent fill, replacing the solid hex shape — the ring animates and scales with `--ring-p1` / `--ring-p2` CSS variables (0–100%, updated by `renderGroupCaptureBadges()`). Unclaimed remainder (100% - p1% - p2%) is transparent, no black fill. Requires a Firefox/Chromium patch for `conic-gradient` to work correctly with both `padding-box` and `border-box` backgrounds simultaneously (raw colour in padding-box invalidates the whole declaration per CSS spec; see findings.md [F2] for the fix: wrap the colour in a `linear-gradient` layer).
- **CHANGED**: `mobile/sw.js` service-worker cache version bumped `pme-mobile-v12` → `v14` to clear stale precache entries and refresh the audio manifest (see deployment notes in CLAUDE.md about why a version bump is needed for any media addition).
- **CHANGED**: `package.json` + `mobile/main.js` `GAME_VERSION` → `2.6.0`.

### Fixed
- **FIXED**: Group-capture sound had been placed incorrectly via log-string-match: `doEndPhase` scanned the log's last 10 entries for text starting with "You hold" and fired `fanfare`, so a capture mid-phase was silent and a fanfare arrived later at the phase boundary, detached from the action and colliding with fanfare's other meaning. Replaced with edge-detect in `renderGroupCaptureBadges()` against a module-level `prevCapturedP1` map (see findings.md [F7] for the discovery). New `group_won` sound replaces fanfare for this signal (see Added section above).

### Technical notes
- **Measured 2026-09-02:** Adding states to a regional-dominance group makes it *harder* to capture (not easier) under the per-state AND-gate. Manufacturing expanded 7→9 dropped its capture rate from 78.7% to 69.4%; Education gained Assam, went 80.5%→76.7%. Every added state is another veto point; difficulty is driven by member *count*, not by total seat volume (see findings.md [F6]).
- **Measured 2026-09-02:** Politician win rate is driven by their agenda kit's seat-weighted national pull (Spearman rho=0.73 vs. win rate, p=0.0002), not home state (rho=0.22, p=0.35). Rahul Gandhi's low win rate (14.3%) is explained by his agenda kit netting -4bps in his own home state (Uttar Pradesh), not his power's implementation — the power itself deploys near-optimally (see findings.md [F4] and [F5]). This reverses an earlier false diagnosis that home-state popularity was causal; end-of-game measurements are outcomes of winning, not causes.
- **Styling caveat:** CSS `background` shorthand with both a colour and a gradient layer requires the colour to wrap in a gradient for validity — `linear-gradient(colour 0 0) padding-box, conic-gradient(...) border-box` works; `colour padding-box, conic-gradient(...) border-box` silently computes to `backgroundImage: none` with no console warning or parse error (see findings.md [F2]).
...
### Changed
- **CHANGED**: `mobile/privacy.html` contact email filled in (`pradhanmantrielectionsgame@gmail.com`), replacing the placeholder. The privacy policy is now submission-ready for the Play Store listing.
...
### Fixed
- **FIXED**: Taps felt laggy even though nothing on screen was slow. Every control was bound to `click`, which cannot fire until the finger *lifts* — so the sound and FX for an invest were gated behind the player's own press duration (~70–120 ms of finger dwell). Measured event order in WebKit: `pointerdown` → `touchend` → `click`, with click landing at dwell + ~7 ms. New `fastTap()` helper (`mobile/main.js`) binds `pointerdown` instead, with a `click` fallback (deduped on a 700 ms window) so keyboard activation still works on real `<button>`s. Applied to the map, the five quick-invest buttons (Delhi/Goa/Kerala/Small UTs/NE 8), the agenda tray, the rally/special/nationwide buttons, and the group chips — **not** the politician carousel, whose `click` binding is what keeps a swipe from firing a card. This class of latency is invisible to Playwright, whose synthetic taps press and release in the same tick.
- **FIXED**: The welcome screen's "wait for portraits" gate was a guaranteed 3.5-second stall that loaded nothing. It waited on the carousel's own `<img>` elements, but all 21 are `loading="lazy"` inside `#selectOverlay`, which is `hidden` behind the welcome screen — and a browser never fetches a lazy image in a hidden subtree, so `load` never fired and every boot sat out the full `setTimeout(resolve, 3500)` fallback. Probed at the moment the button unlocked: 0 of 21 complete, 0 decoded. Replaced with a `new Image()` preload of the first two portraits (ignores visibility, so it actually preloads) and a 1,500 ms straggler guard.
- **FIXED**: `renderRallyTokens()` interleaved `getBoundingClientRect()` reads with `appendChild` writes in one loop, forcing a synchronous layout recalculation per rallied state — measured 2.19 ms vs 0.09 ms batched (24×) at 20 states on desktop WebKit, growing with every rally played. Now reads every rect first, then appends one `DocumentFragment`.
- **FIXED**: The politician-carousel `setInterval` had no stop condition and kept running through an entire match while the select screen was hidden — 21 `localStorage` reads + 21 `JSON.parse`s + 21 `innerHTML` rebuilds per second, for cards nobody could see. Now bails when `#selectOverlay` is hidden.
- **FIXED**: `.fx-flash` animated `width`/`height`/`margin` — layout properties, so every frame of the 450 ms tap-feedback animation forced a layout + paint on the main thread, precisely while the player was tapping fast. Re-authored to animate `transform: scale()` and `opacity` (compositor-only), sized at the end state and scaled down to the start so the final frame is pixel-identical.
- **FIXED**: In `investPaid()`, the sound already fired ahead of `renderAll()` for latency reasons but the visual FX still fired *after* it. Flash, money text, and the state pulse now all precede the re-render alongside the sound.

### Changed
- **CHANGED**: All 21 politician portraits and the welcome background converted from PNG to WebP q80 at identical pixel dimensions — **19.75 MB → 2.05 MB (9.4×)**. They were 8-bit RGB PNGs with no alpha channel: a lossless codec for photographic content. PNG is already compressed, so GitHub Pages' gzip did nothing for it and that 17.7 MB was real wire traffic on every cold visit. Not a downscale — 768 px is already conservative against the 373×170 CSS px render target on a 3× screen.
- **CHANGED**: The two auto-traced party-logo SVGs rasterised to 96 px WebP — **989 KB → 25 KB**. `All_India_Trinamool_Congress_logo_(3).svg` alone was 895 KB (130,474 coordinate points across 7 paths, an Inkscape trace of a bitmap) rendering as a ~30 px badge; rounding coordinates to 2dp only saved 13%, so rasterising was the only real fix. The other three party logos are genuine vector art (8–28 KB) and stay as SVG.
- **CHANGED**: `html2canvas.min.js` (194 KB) is no longer a `<script>` tag in `index.html` — `loadHtml2Canvas()` injects it on the first Share tap. Its only use is the end-of-game declare-card screenshot, which most sessions never reach. It stays precached in `sw.js` deliberately (install runs off the boot critical path), so the share screenshot is still instant and works offline.
- **CHANGED**: `renderGroupCard` and `renderClusterCard` merged into one `renderMemberCard()` — they shared a 14-line verbatim LED-grid block and differed only in header text and whether the pin button shows. The grid rebuild is now guarded by a content key (the same `dataset` trick `syncNewsFeed()` uses), since `renderAll()` ran it on every player tap *and* every AI tick, wiping 5–15 buttons and rebinding a listener on each. The three sibling renderers that repurpose `#cardGroups` (`renderActionInfo`, `renderAgendaCard`, `renderStateCard`) each clear that key, or re-selecting a group after viewing one of them would short-circuit and leave their content on screen.
- **CHANGED**: `renderGroupCaptureBadges()` reads cached chip elements from a `groupChipEls` side map instead of running 15 `document.querySelector('.gchip[data-key="…"]')` calls per render (0.26 ms each pass). Deliberately a side map and **not** a field on the group objects: `game` gets `structuredClone()`d by `planAITickPacing()`, a DOM node is not cloneable, and hanging one off `game.groups` throws there — which leaves `game.rng` nulled and breaks the AI for the rest of the match.
- **CHANGED**: `scripts/shrink-images.py` rewritten to emit WebP rather than re-optimising PNGs in place; new `scripts/rasterize-logos.js` renders the two oversized SVGs via the Playwright already in devDependencies (cairosvg needs a libcairo DLL Windows doesn't ship).
- **CHANGED**: Service worker cache version bumped `pme-mobile-v7` → `v8`, required because media is cache-first — without it, returning players would keep the old PNGs indefinitely.

### Removed
- **REMOVED**: `assets/icons/INDIA_V2.svg` and `INDIA_V3_smaller_viewbox.svg` (204 KB each) and `pme-icon-1024.png` — unreferenced anywhere, but `deploy-mobile.js` copies `assets/` wholesale, so they were shipping to the live site.
- **REMOVED**: 10 dead CSS rules (`.players-mini`, `.pmini*`) from `mobile/index.html` — zero references in any HTML or JS.
- **REMOVED**: `mobile/index-redesign-c.html` and `manifest-redesign-c.json` moved to `design/prototypes/` (relative asset paths rebased for the new depth). The dark-theme comparison prototype is history, not runtime, and doesn't belong in the folder that ships.

### Notes
Measured clean and deliberately left alone: the game engine (`investCash` with its full payout sweep is 0.27 µs, `dominanceActive` across all 15 groups × 2 players 11.4 µs, `nationalSeats` 19.1 µs, `aiStep` 26.0 µs); `planAITickPacing`'s `structuredClone` + 500-step dry run (0.3 ms, ten times a game); the inline India map SVG (198 KB of path data already at 1 decimal place — coordinate rounding saves *zero* bytes, and it gzips to 65 KB with the rest of `index.html`); and the service worker's network-first/cache-first split, which is correct as written.
...
### Changed
- **CHANGED**: Wired the real Buy Me a Coffee link (`https://buymeacoffee.com/pradhanmantri`) into the "Support the developer" tip-jar links on the welcome screen and end-game card, replacing the `REPLACE_ME` placeholder in `mobile/main.js`.
